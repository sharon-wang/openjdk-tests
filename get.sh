...
OPENJ9_REPO="https://github.com/sharon-wang/openj9.git"
OPENJ9_SHA=""
OPENJ9_BRANCH="testForPR5353"
VENDOR_REPOS=""
VENDOR_SHAS=""
VENDOR_BRANCHES=""
VENDOR_DIRS=""
...
getBinaryOpenjdk()
{
	...
	if [ "${download_url}" != "" ]; then
		...
		do
			set +e
			echo "curl -OLJks --retry 5 --retry-delay 300 ${curl_options} $file"
			curl -OLJks --retry 5 --retry-delay 300 ${curl_options} $file
			download_exit_code=$?
			if [ $download_exit_code != 0 ]; then
				echo "Error code: $download_exit_code"
